...
#                print ("emgc",emgc)
    ...
    def gettemp ():
        global temp
        while True:
            temp = round(random.uniform(32.01,37.00),2)
            sleep(20)
    ...
    def urlget():
        ...
        while True:
            if scount == 20:
                ...
                data = {'med': mediafreq(ffemgs),'emg': sendemg,'femg': ffemgs,'temp': temp}
                scount = scount +1
            elif scount == 40:
                sendemg = emgdenoise(emg)
                data = {'med': mediafreq(ffemgs),'emg': sendemg,'femg': ffemgs,'plus': plu,'spo': spo}
                scount = 0
            else:
                sendemg = emgdenoise(emg)
                ffemgs = emgfft(sendemg)
                data = {'med': mediafreq(ffemgs),'femg': ffemgs,'emg': sendemg,}
                scount = scount +1
            urllib.request.urlopen(url+parse.urlencode(data,doseq=True))
            print ("URL is",parse.urlencode(data))
            sleep(1)
...
#   t5 = threading.Thread(target=showpic)
